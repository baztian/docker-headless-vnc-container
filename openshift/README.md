...
The following content uses as example the image `baztian/headless-vnc-container`.
...
As soon as you are logged in and selected your oc project, you can simple run the image by using the configuration `openshift.headless-vnc.run.yaml`:
    ...
    oc process -f openshift.headless-vnc.run.yaml -v APPLICATION_NAME=myrunonlypod IMAGE=baztian/headless-vnc-container | oc create -f -
